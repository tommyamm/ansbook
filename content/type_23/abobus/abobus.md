...
В коде есть важная особенность — значение `flag` передаётся **по значению**, а не по ссылке. Это означает, что каждая ветвь рекурсии имеет свою копию счётчика, что корректно для данной задачи.